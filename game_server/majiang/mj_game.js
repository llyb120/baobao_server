...
class MJGame extends GameBase{
    ...
    sync(chairId = -1) {
        ...
        if(this._state == 'game') {
            ...
            this._game.players.forEach(function (userId, index) {
                if(chairId !== undefined && index != chairId){
                    return;
                }
                var data = {
                    //游戏状态
                    ...
                    //上一个出牌的人
                    lastChair: self._lastChair,
                    //上一个人出的上一张牌
                    lastCard: self._lastCard,
                    //用户持有的组合
                    combo: self._combo,
                    //下炮子 0 - 4（1-5倍）
                    pao : self._pao,
                    ...
                    showAllCards : self._showAllCards,

                    zhuang : self._zhuang,

                    leftCardCount : self._cards.length,

                    gameScore : self._lastGameScore,
                    mopai : self._mopai

                };
                if(self._showAllCards){
                    data.handCards = self._handCards;
                }
                else{
        ...
        if(this._actionCache.hu.length){
            ...
            this._game.players.forEach(function (player,index) {
                 ...
                 var score =
                     ...
                     (
                         //普通平胡不变,只炸不胡炮子分*2
                        (self._pao[index] + self._pao[chairId]) * constant.SCORE_BASE
                         //自摸翻倍
                         * (zimo ? 2 : 1)
                         +
    ...
    xipai(){
        ...
        //8,8,12,13,17,18,20,21,21,25,26,26,31
        //test
    ...
    checkHu(chairId,card,add = false){
        ...
        //}
        //七对
        var qiduifan = this._roomInfo.gameConfig.qiduifan;
        if(qiduifan>0){
            if(Object.keys(this._tingMap[chairId]).length > 0 && this._tingMap[chairId][card] != undefined && this._tingMap[chairId][card].jiang == -1 && flag){
                return true;
            }
        }
        //258做将
        ...
        if(jiang258){
            //console.log(Object.keys(this._tingMap[chairId]));
            //if(Object.keys(this._tingMap[chairId]).length > 0){
            //    console.log(this._tingMap[chairId][card].jiang);
            //    var jiang = this._tingMap[chairId][card].jiang;
            //}
            if(Object.keys(this._tingMap[chairId]).length > 0){
                console.log(Object.keys(this._tingMap[chairId]).length);
                console.log(this._tingMap[chairId][card]);
                //console.log(Object.keys(this._tingMap[chairId][card]).length);
            }
            if(Object.keys(this._tingMap[chairId]).length > 0 && this._tingMap[chairId][card] != undefined && jiang258Card.indexOf(this._tingMap[chairId][card].jiang) >0 && flag){
                return true;
            }
            else{
                return false;
        ...
        return flag
                &&(
                    //需要这张牌
                    this._tingMap[chairId][card]
                    ||
                    //红中癞子的情况下，只要可以胡，任意一张红中就可以
                    (this._roomInfo.gameConfig.hongzhong && card == this._magic && Object.keys(this._tingMap[chairId]) > 0)
                )
                ;
    }
